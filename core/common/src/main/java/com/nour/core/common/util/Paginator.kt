...
class Paginator<T, Page>(
    ...
    private val firstPageKey: String? = null,
    ...
    private val _items = MutableStateFlow<ResponseState<List<T>>>(ResponseState.Success(emptyList()))
    val items: Flow<ResponseState<List<T>>> = _items.asStateFlow()

    private var nextPageKey: String? = firstPageKey
    ...
    suspend fun loadInitial(force: Boolean = false) {
        ...
        when (val result = initialLoad(initialLoadSize)) {
            is ResponseState.Success -> {
                ...
                nextPageKey = getNextCursor(page)
    ...
    suspend fun loadNextPage() {
        ...
        when (val result = loadMore(nextPageKey, pageSize)) {
            is ResponseState.Success -> {
                val page = result.data
